...
# Usage
# python evaluation/eval.py --ground_truth <ground_truth_jsonl> -i <input_folder> -o <output_jsonl>
...
def evaluate(predictions_path: str, ground_truth_path: str, tiou_threshold: float = 0.5, gt_window_seconds: float = 2.0):
    ...
    with open(ground_truth_path, "r") as f:
        for line in f:
            data = json.loads(line)
            # Adapt to the format of our_dataset.jsonl
            if "id" in data and "h" in data: # Format is {"id": "...", "h": [{"t": ...}]}
                video_id = data["id"]
            ...
            elif "video_name" in data and "timestamps" in data: # Original expected format
                gt_timestamps = data["timestamps"]
                gt_segments = [[t - gt_window_seconds, t + gt_window_seconds] for t in gt_timestamps]
                gts[data["video_name"]] = {"segments": gt_segments, "timestamps": gt_timestamps}
